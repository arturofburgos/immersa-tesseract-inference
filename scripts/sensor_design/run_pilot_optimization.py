...
def run_start(
    ...
    """Optimize one starting layout and report the outcome.

    Kept out of the caller's loop so the ``objective`` closure captures plain
    function locals. Defining it inside a loop would close over the loop
    variables by reference, which ruff flags as B023 and which would silently
    misbehave if the closure ever outlived its iteration.
    """
    tau = calibration["tau"]
    ...
    initial_penalty, _ = separation_penalty(start, lambda_separation=lambda_separation)
    ...
    final_penalty, _ = separation_penalty(final, lambda_separation=lambda_separation)
    ...
    print(
        f"  per objective+grad : {wall_time / max(evaluations['count'], 1):.3f} s",
        flush=True,
    )
    print(flush=True)
    ...
    return {
        ...
        "seconds_per_evaluation": wall_time / max(evaluations["count"], 1),
    }


def report_calibration(calibration: dict[str, float], elapsed: float) -> None:
    """Print the frozen tau and separation weight with their diagnostics."""
    print("Calibration at the baseline layout:", flush=True)
    print(f"  retained pairs        : {calibration['n_pairs']}", flush=True)
    print(f"  min pair distance     : {calibration['min_distance']:.8f}", flush=True)
    print(f"  10th pair distance    : {calibration['rank_distance']:.8f}", flush=True)
    print(
    ...
    print(
        f"  tau (frozen)          : {calibration['tau']:.8f}  "
        f"(N_eff = {calibration['effective_pairs']:.4f})",
        flush=True,
    )
    print(f"  top-1 softmin weight  : {calibration['top1_weight']:.6f}", flush=True)
    print(f"  top-10 cumulative wt  : {calibration['top10_weight']:.6f}", flush=True)
    print(
        f"  lambda_separation     : {calibration['lambda_separation']:.8f}",
        flush=True,
    )
    print(f"  calibration wall time : {elapsed:.2f} s")
    print(flush=True)
...
def main() -> None:
    ...
    with SensorDesignPipeline() as pipeline:
        ...
        report_calibration(calibration, time.perf_counter() - started)

        rows = [
            run_start(pipeline, name, start, calibration)
