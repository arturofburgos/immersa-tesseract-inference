...
def load_predictor():
    """
    Load the final V3 model and construct a differentiable
    physical-space map.
    ...
    def predict_physical(
        ...
        """
        Predict physical velocities from
        [alpha_deg, x, y].
        """
...
def central_difference(
    ...
    """
    Compute a central finite-difference derivative
    of all ten model outputs.
    """
...
def main() -> None:
    ...
    for (
        ...
        print(
            "Prediction RMS: "
            f"{np.sqrt(np.mean(prediction**2)):.6e}"
        )
        ...
        for (
            ...
            print(
                "  BEST: "
                f"eps={best_epsilon:.1e} "
                f"| relL2="
                f"{best_relative_error:.6e}"
            )
    ...
    print(
        "Best relative error, mean: "
        f"{np.mean(best_errors):.6e}"
    )

    print(
        "Best relative error, max:  "
        f"{np.max(best_errors):.6e}"
    )

    print(
        "Best cosine similarity, min: "
        f"{np.nanmin(best_cosines):.9f}"
    )
