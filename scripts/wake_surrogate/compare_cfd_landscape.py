...
def load_cfd_ns2_landscape(
    ...
    if not ns2_rows:
        raise RuntimeError(
            "No Ns=2 rows found in "
            "CFD landscape CSV."
        )

    print(
        f"Found {len(ns2_rows)} "
        f"CFD rows for Ns=2."
    )
    ...
    j_per_scalar = j_per_scalar[
        order
    ]

    print(
        f"CFD Ns=2 alpha range: "
        f"{alpha.min():.1f} to "
        f"{alpha.max():.1f} deg"
    )
...
def load_cfd_truth_measurements() -> np.ndarray:
    """
    Extract real-CFD measurements at the two Ns=2
    sensor positions.
    ...
    with np.load(
        ...
        sensor_x_all = np.asarray(
            data[
                "sensor_x"
            ]
        )

        sensor_y_all = np.asarray(
            data[
                "sensor_y"
            ]
        )
    ...
    if not np.allclose(
        ...
        raise RuntimeError(
            "Test-file observation times do not "
...
def make_predictor():
    """
    Load the final V3 surrogate and return a physical-space
    Ns=2 prediction function.
    """
...
def main() -> None:
    ...
    print(
        "WAKE SURROGATE VS REAL-CFD INVERSE LANDSCAPE"
    )
    ...
    print(
        f"  RMSE / CFD range:  "
        f"{100.0 * normalized_curve_rmse:.2f}%"
    )
    ...
    ax.set_title(
        "Inverse-objective landscape: "
        "Immersa CFD vs WakeSurrogate V3"
    )
    ...
    figure_png = (
        RESULTS_ROOT
        / "cfd_vs_surrogate_landscape.png"
    )

    figure_pdf = (
        RESULTS_ROOT
        / "cfd_vs_surrogate_landscape.pdf"
    )
    ...
    print(
        f"\nSaved surrogate landscape: "
        f"{output_csv}"
    )

    print(
        f"Saved landscape metrics:   "
        f"{metrics_csv}"
    )

    print(
        f"Saved local minima:        "
        f"{minima_csv}"
    )
