...
MODEL_ROOT = Path("models/wake_surrogate")

RESULTS_ROOT = Path("results/wake_surrogate/model_selection")
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Evaluate one WakeSurrogate version on the held-out validation AoAs."
        )
    )
    ...
    parser.add_argument(
        ...
        help=("WakeSurrogate version to evaluate."),
    )
...
def alpha_tag(
    ...
    return f"{alpha_deg:05.1f}".replace(
        ".",
        "p",
    )
...
def validate_validation_files(
    ...
    for path in validation_files:
        ...
        loaded_alphas.append(alpha_deg)
...
def plot_case(
    ...
    time_value = SENSOR_TIMES[time_index]
    ...
    err_ux = pred_ux - true_ux

    err_uy = pred_uy - true_uy
    ...
    output_path = results_root / (f"validation_alpha_{alpha_tag(alpha_deg)}.png")
...
def main() -> None:
    ...
    model_root = MODEL_ROOT / config["directory"]

    results_root = RESULTS_ROOT / config["directory"]
    ...
    print("WAKE SURROGATE VALIDATION")
    ...
    validation_files = sorted((DATA_ROOT / "validation").glob("*.npz"))

    if not validation_files:
        raise RuntimeError("No validation files found.")

    validate_validation_files(validation_files)
    ...
    for path in validation_files:
        with np.load(path) as data:
            ...
            inputs_physical = np.asarray(data["inputs"])

            targets_physical = np.asarray(data["targets"])

            sensor_x = np.asarray(data["sensor_x"])

            sensor_y = np.asarray(data["sensor_y"])

            file_times = np.asarray(data["sensor_times"])
        ...
        if not np.allclose(
            ...
            raise RuntimeError(f"Unexpected observation times in {path}.")

        # ----------------------------------------------------
        # Normalize inputs
        # ----------------------------------------------------

        inputs_normalized = normalize_inputs(inputs_physical)
        ...
        truth = targets_physical.reshape(
            -1,
            len(SENSOR_TIMES),
            2,
        )

        prediction = prediction_physical.reshape(
            -1,
            len(SENSOR_TIMES),
            2,
        )
        ...
        row = {
            ...
            "ux_rmse": ux_rmse,
            "uy_rmse": uy_rmse,
            "relative_l2_percent": (100.0 * rel_error),
            "ux_relative_l2_percent": (100.0 * ux_rel_error),
            "uy_relative_l2_percent": (100.0 * uy_rel_error),
        }
        ...
        for (
            time_index,
            time_value,
        ) in enumerate(SENSOR_TIMES):
            row[f"rmse_t_{time_value:.1f}"] = rmse(
                prediction[
                    :,
                    time_index,
                    :,
            ...
            row[f"relative_l2_percent_t_{time_value:.1f}"] = 100.0 * relative_l2(
                prediction[
                    :,
                    time_index,
                    :,
    ...
    output_csv = results_root / "validation_metrics.csv"
    ...
    mean_rel = float(np.mean([row["relative_l2_percent"] for row in rows]))

    mean_ux_rel = float(np.mean([row["ux_relative_l2_percent"] for row in rows]))

    mean_uy_rel = float(np.mean([row["uy_relative_l2_percent"] for row in rows]))

    print("\n" + "-" * 78)

    print(f"Mean validation RMSE:        {mean_rmse:.6f}")

    print(f"Mean validation relative L2: {mean_rel:.2f}%")

    print(f"Mean ux relative L2:         {mean_ux_rel:.2f}%")

    print(f"Mean uy relative L2:         {mean_uy_rel:.2f}%")

    print(f"\nSaved metrics: {output_csv}")

    print(f"Saved figures: {results_root}")
