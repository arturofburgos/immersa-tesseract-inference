...
MODEL_CONFIGS = {
    "v1": {
        ...
        "directory": (MODEL_ROOT / "v1_spatial_fourier"),
    },
    "v2": {
        "label": "V2 HIGH-AOA REFINEMENT",
        "directory": (MODEL_ROOT / "v2_high_aoa_refinement"),
    },
    "v3": {
        "label": "V3 BROAD AOA REFINEMENT",
        "directory": (MODEL_ROOT / "v3_broad_aoa_refinement"),
    },
}
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description=("Train one version of the WakeSurrogate model.")
    )
    ...
    parser.add_argument(
        ...
        help=("WakeSurrogate version to train."),
    )
    ...
    parser.add_argument(
        "--overwrite",
        action="store_true",
        help=("Allow overwriting an existing best_model.eqx."),
    )
...
def load_split(
    ...
    if not files:
        raise RuntimeError(f"No files found in {split_dir}.")
    ...
    for path in files:
        with np.load(path) as data:
            ...
            all_inputs.append(np.asarray(data["inputs"]))

            all_targets.append(np.asarray(data["targets"]))

            selected_alphas.append(alpha_deg)

    if not all_inputs:
        raise RuntimeError(f"No matching files found for split '{split}'.")
    ...
    selected_alphas = sorted(set(selected_alphas))
...
def validate_training_alphas(
    ...
    """Verify that exactly the requested training AoAs were loaded."""
    selected = np.asarray(
        selected_alphas,
        dtype=np.float64,
    )
...
class WakeSurrogate(eqx.Module):
    ...
    def fourier_features(
        ...
        """Keep alpha smooth and Fourier-encode only spatial coordinates."""
...
def train_step(
    ...
    x_batch: jax.Array,
    y_batch: jax.Array,
) -> tuple[WakeSurrogate, optax.OptState, jax.Array]:
...
def main() -> None:
    ...
    model_config = MODEL_CONFIGS[args.version]

    train_alphas = TRAIN_ALPHAS_BY_VERSION[args.version]

    if args.output_dir is None:
        model_root = model_config["directory"]
    ...
    best_model_path = model_root / "best_model.eqx"
    ...
    if best_model_path.exists() and not args.overwrite:
        raise RuntimeError(
    ...
    print(f"WAKE SURROGATE TRAINING - {model_config['label']}")

    print("=" * 78)

    print(f"Version          : {args.version}")

    print(f"Output directory : {model_root}")
    ...
    print(f"Fourier freq.    : {np.asarray(FOURIER_FREQUENCIES).tolist()}")
    ...
    print(f"Training AoAs ({len(selected_train_alphas)}):")

    print(selected_train_alphas)

    print()

    print(f"Validation AoAs ({len(selected_valid_alphas)}):")

    print(selected_valid_alphas)
    ...
    x_train = normalize_inputs(x_train)

    x_valid = normalize_inputs(x_valid)
