...
def parse_arguments() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description=("Generate CFD data for one WakeSurrogate training-data version.")
    )
    ...
    parser.add_argument(
        ...
        help=("WakeSurrogate dataset version to generate."),
    )
...
OUTPUT_ROOT = Path("data/wake_surrogate")
...
def generate_case(
    ...
    output_path = output_dir / (f"alpha_{alpha_tag(alpha_deg)}.npz")
    ...
    if measurements.shape != expected_shape:
        raise RuntimeError(
    ...
    if not np.all(np.isfinite(measurements)):
        raise RuntimeError(f"Non-finite measurements for alpha={alpha_deg}.")
...
def main() -> None:
    ...
    train_alphas = list(TRAIN_ALPHAS_BY_VERSION[dataset_version])

    validation_alphas = list(VALIDATION_ALPHAS)

    test_alphas = list(TEST_ALPHAS)
    ...
    n_train_samples = len(train_alphas) * SENSOR_X.size

    n_validation_samples = len(validation_alphas) * SENSOR_X.size

    n_test_samples = len(test_alphas) * SENSOR_X.size

    n_total_samples = n_train_samples + n_validation_samples + n_test_samples
    ...
    print("WAKE SURROGATE DATASET GENERATION")

    print("=" * 78)

    print(f"Dataset version  : {dataset_version}")

    print(f"Training AoAs    : {train_alphas}")

    print(f"Validation AoAs  : {validation_alphas}")

    print(f"Test AoAs        : {test_alphas}")

    print(f"\nTraining AoA count   : {len(train_alphas)}")

    print(f"Validation AoA count : {len(validation_alphas)}")

    print(f"Test AoA count       : {len(test_alphas)}")

    print(f"\nSpatial grid     : {len(X_VALUES)} x {len(Y_VALUES)}")

    print(f"Sensor locations : {SENSOR_X.size}")
    ...
    print(f"Total ML samples   : {n_total_samples:,}")

    print(f"\nTotal requested CFD runs : {len(jobs)}")

    print(f"Workers                  : {MAX_WORKERS}")

    print(f"CFD resolution           : h={H}, dt={DT}, tf={TF}, Re={RE}")

    print(f"Snapshot frequency       : {SNAPSHOT_FREQ}")

    print()

    print("Existing dataset files are skipped.")
    ...
    with ProcessPoolExecutor(
        ...
        for future in as_completed(futures):
            ...
            try:
                ...
                print(
                    f"[FAILED   ] {split:10s} alpha={alpha:7.3f}: {exc!r}",
                    flush=True,
                )
    ...
    print(f"Requested runs  : {len(jobs)}")
