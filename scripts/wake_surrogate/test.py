...
RESULTS_ROOT = Path("results/wake_surrogate/final_validation/test_alpha63")
...
PLOT_TIME_INDICES = list(range(len(SENSOR_TIMES)))
...
def time_tag(
    ...
    return f"{time_value:04.1f}".replace(
        ".",
        "p",
    )
...
def plot_case_all_times(
    ...
    for time_index in time_indices:
        ...
        ux_vmin = min(
            float(np.min(true_ux)),
            float(np.min(pred_ux)),
        )

        ux_vmax = max(
            float(np.max(true_ux)),
            float(np.max(pred_ux)),
        )

        uy_vmin = min(
            float(np.min(true_uy)),
            float(np.min(pred_uy)),
        )

        uy_vmax = max(
            float(np.max(true_uy)),
            float(np.max(pred_uy)),
        )
        ...
        ux_error_limit = max(
            abs(float(np.min(err_ux))),
            abs(float(np.max(err_ux))),
        )

        uy_error_limit = max(
            abs(float(np.min(err_uy))),
            abs(float(np.max(err_uy))),
        )
        ...
        axes[
            0,
            1,
        ].set_title(r"Surrogate $\hat{u}_x$")
        ...
        axes[
            0,
            2,
        ].set_title(r"Error $\hat{u}_x-u_x$")
        ...
        axes[
            1,
            1,
        ].set_title(r"Surrogate $\hat{u}_y$")
        ...
        axes[
            1,
            2,
        ].set_title(r"Error $\hat{u}_y-u_y$")
        ...
        output_path = RESULTS_ROOT / (
            f"test_alpha_{alpha_tag(alpha_deg)}_t_{time_tag(time_value)}.png"
        )
...
def main() -> None:
    ...
    expected_test_alpha = float(TEST_ALPHAS[0])
    ...
    print("WAKE SURROGATE FINAL TEST")
    ...
    print(f"Test AoA         : {expected_test_alpha}")
    ...
    if len(test_files) != 1:
        raise RuntimeError(f"Expected exactly one test file, found {len(test_files)}.")
    ...
    for (
        ...
        row[f"rmse_t_{time_value:.1f}"] = time_rmse

        row[(f"relative_l2_t_{time_value:.1f}_percent")] = 100.0 * time_rel

    # ========================================================
    # Save metrics
    # ========================================================

    output_csv = RESULTS_ROOT / "test_metrics.csv"
    ...
    print(f"Saved test metrics: {output_csv}")

    print(f"Saved test figures: {RESULTS_ROOT}")
