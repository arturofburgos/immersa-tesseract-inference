...
This problem does not correspond to a single homogeneous chain `T1 → T2 → T3 → T4`. Instead, the project uses three distinct computational routes. The first is used for physical AoA inference, while the other two are complementary routes for sensor design: one to search for a layout cheaply, one to verify it against real CFD.
...
**Cheap design route — `T3 → T4`.** Optimizing probe positions means re-scoring a candidate layout against the whole set of candidate AoAs at every optimizer step, which is far too expensive to do with CFD in the loop. `WakeSurrogate` was trained to approximate the observable map represented by `T1 → T2`: given `(α, x, y)`, it predicts `ux, uy` at the five observation times, anywhere in the trained sensor-design region. During optimization it is queried only at the current candidate sensor coordinates, so it bypasses both the CFD solve and the sampling operation in `T2`. `SensorArrayDesign` then scores that batch: it measures how far apart the two most similar-looking angles are and returns a single differentiable number `D_τ` for the optimizer to maximize. The worst case is smoothed rather than taken literally, so the gradient does not jump each time a different pair becomes the hardest. One objective-and-gradient evaluation spanning all 66 candidate AoAs takes under a second, whereas obtaining the same 66 flow fields from `T1` would take roughly an hour of CFD. That margin is what makes multistart gradient optimization over the design box affordable, so this is the route that actually moves the probes. Conceptually `T3 ≈ T2 ∘ T1`, but only for the observable map needed for experiment design, never for the full CFD state.


**High-fidelity sensor-design route — `T1 → T2 → T4`.** A layout that scores well under `T3` is only a proposal, so it is re-scored on real CFD using the identical `T4` objective. Which layouts get checked is decided before any CFD is run. The surrogate campaign optimizes from multiple starts, and the layout with the highest `T4` discrimination is frozen. No physical result influences that choice, so the mechanistic evaluation stays an honest test rather than a selection step. Only the frozen layout and the conventional baseline are then scored on real CFD. This route reuses the same mechanistic `T1 → T2` measurement path, but sends a measurement batch spanning candidate AoAs to `SensorArrayDesign` instead of to the inverse optimizer. Crucially, it does **not** rerun the CFD solver every time a probe moves. Probes are passive: moving one does not change the flow. Persisted `T1` fields can therefore be sampled by `T2` at any new sensor positions, the whole dependence of the design score on the sensor coordinates arises inside `T2`, and the gradient flows back through `T2 → T4` alone. No derivative of `T1` with respect to sensor position is ever required, which is also what allows this same route to *refine* a design directly on the mechanistic objective.
...
Without Tesseract, the application layer would have to bridge the Julia and Python execution boundary itself and implement the derivative exchange between components by hand, for both the mechanistic and the surrogate paths. Here those concerns stay local to each component, and the application only composes their declared differentiable interfaces, which is what makes swapping the surrogate route for the mechanistic one a change of wiring rather than a rewrite.
