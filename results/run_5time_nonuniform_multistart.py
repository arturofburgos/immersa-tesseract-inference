...
OUTPUT_DIR = Path("results/5time_nonuniform_multistart")
...
KNOWN_MINIMA = {
    "low_false_basin": 51.0,
    "true_basin": 63.0,
    "high_false_basin": 76.0,
}
...
def run_initial_condition(
    ...
    log_path = OUTPUT_DIR / f"initial_{tag}_deg.log"
    ...
    with log_path.open(
        ...
        with (
            ...
            print("=" * 76, flush=True)
            print(
                "NOISE-FREE MULTI-START AoA INFERENCE",
                flush=True,
            )
            print("=" * 76, flush=True)
            ...
            print(
                f"sensor         = ({SENSOR_X[0]:.2f}, {SENSOR_Y[0]:.2f})",
                flush=True,
            )
...
def main() -> None:
    ...
    print("=" * 84, flush=True)
    print(
        "PARALLEL NOISE-FREE MULTI-START AoA STUDY",
        flush=True,
    )
    print("=" * 84, flush=True)
    ...
    print(f"sensor          = ({SENSOR_X[0]:.1f}, {SENSOR_Y[0]:.1f})")
