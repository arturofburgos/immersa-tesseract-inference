...
new:
	...
	if [ -n "$(RECIPE)" ]; then \
		echo "Recipe: $(RECIPE)"; \
		tesseract init --name "immersa_tesseract_inference_$$TESS_SLUG" --target-dir "$$TESS_DIR" --recipe "$(RECIPE)"; \
	else \
		tesseract init --name "immersa_tesseract_inference_$$TESS_SLUG" --target-dir "$$TESS_DIR"; \
	fi; \
...
test:
	@run_tesseract_tests() { \
		...
		for test_file in "$$tess_dir"/test_cases/*.json; do \
			...
			result=$$(tesseract run immersa_tesseract_inference_$$tess_slug test @$$test_file); \
			echo "$$result"; \
...
gen-tests:
	...
	TESS_IMAGE="immersa_tesseract_inference_$$TESS_SLUG"; \
...
run:
	@echo "Running app..."
	@immersa_tesseract_inference $(filter-out $@,$(MAKECMDGOALS))
